...
The `MSstatsConvert` package is a member of the `MSstatst` family of packages.
...
The following column are required.
  ...
  by `MSstats` and `MSstatsTMT` packages is done separately for each protein,
  ...
  endogenous peptide (indicated by value "L") or reference peptide (indicated by value "H"),
  ...
  - `Intensity`: column that stores untransformed (in particular, no log transformation) measurements of feature abundance in a given Run (and Channel in TMT case). 
...
MSstatsLogsSettings(use_log_file = TRUE, append = FALSE) # default - creates a new file
MSstatsLogsSettings(use_log_file = TRUE, append = TRUE, 
                    log_file_path = "log_file.log") # default - creates a new file
MSstatsLogsSettings(use_log_file = FALSE, append = FALSE) # switches logging off
MSstatsLogsSettings(use_log_file = FALSE, verbose = FALSE) # switches off logs and messages
```
...
This function accepts several parameters, and each corresponds to a preprocessing step.
  ...
  `handle_features_with_few_measurements` should be equal "remove" or "keep". In the first case,
  feature that have less than three measurements across runs (or channels in a run for TMT data)
  will be removed. "keep" means that only features with no non-missing measurements will be removed. The `summarize_multiple_psms` element should be a function that will be used to aggregate duplicated feature measurements within a single MS run,
...
For label-free and SRM data, it means that fractionation (or technical replicates) will be detected and features measured in multiple fractions (overlapped) will be assigned to a unique fraction. Then, the data will be adjusted so that within each fraction, every feature has a row for each run. If the intensity value is missing, it will be denoted by `NA`.
